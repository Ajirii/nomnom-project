...
export const getRecipeByIngredients = async (ingredients: string, cuisine: string, strict: string) => {
    ...
    try {
        ...
        const content: {title: string, ingredients: string[], instructions: string[], cookingMinutes: number, servings: number, notes: string} = JSON.parse(rawResponse);
...
async function storeRecipe(recipe:{title: string, ingredients: string[], instructions: string[], cookingMinutes: number, servings: number, notes: string}) {
    try {
...
async function doesRecipeExist(recipe: {title: string, ingredients: string[], instructions: string[], cookingMinutes: number, servings: number, notes: string}){
    try{
        ...
        if (existingRecipe) {
            ...
            const normalizedExistingInstructions = existingRecipe.instructions.map((instruction: string) => instruction.toLowerCase()).join(" ");
